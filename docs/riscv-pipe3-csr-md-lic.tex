...
The processor executes at top 1 CPI (clocks per instruction) with a sequential flow of instructions. A preliminary test with CoreMark showed an average CPI of 1.53 with a throughput of 2.22 coremark/MHz.
...
\item The memory unit interfaces with memory and CSR.
\end{itemize}
...
\item \lstinline|mxspeed| -- this custom CSR with address 0xfc1 is read-only and contains the system frequency in Hz of the synthesized processor (see Section~\ref{sec:hardware}).
\end{itemize}
...
\begin{table}[!ht]
\caption{FPGA resource utilization for the DE0-CV board.}
\label{tab:util}
\centering
\begin{tabular}{lllll}
\toprule
...
Freq & 77.97   & 73.07   & 78.25     & 76.13     \\
ALM  & 3377    & 2735    & 3466      & 2739      \\
