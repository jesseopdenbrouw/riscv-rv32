...
\item \lstinline|mxspeed| -- this custom CSR with address 0xfc1 is read-only and contains the system frequency of the synthesized processor (see Section~\ref{sec:hardware}).
\end{itemize}
...
\item \lstinline|ctor_c| -- C test to check if global constructors (and destructors) are called upon program execution. Uses UART1. Works on the board.
